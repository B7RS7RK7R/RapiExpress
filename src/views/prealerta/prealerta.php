...
$(document).ready(function() {
    ...
    // ============================
    function validarCampo($campo, valido, mensaje = '') {
        if (valido) {
    ...
    function validarFormularioPrealerta($form) {
        ...
        if (!cliente) return alerta('Debe seleccionar un cliente.');
        if (!tienda) return alerta('Debe seleccionar una tienda.');
        if (!casillero) return alerta('Debe seleccionar un casillero.');
        if (!sucursal) return alerta('Debe seleccionar una sucursal.');
        if (!/^[A-Za-z0-9\-]{3,50}$/.test(tracking)) {
            return alerta('El tracking solo puede contener letras, números o guiones (3 a 50 caracteres).');
        }
        if (!piezas || piezas <= 0) return alerta('Debe ingresar un número válido de piezas mayor que 0.');
        if (!peso || peso <= 0) return alerta('Debe ingresar un peso válido mayor que 0.');
        if (descripcion.length > 255) return alerta('La descripción no puede exceder los 255 caracteres.');

        return true;
    }

    function alerta(msg) {
        Swal.fire('Campo inválido', msg, 'warning');
        return false;
    }
    ...
    $('#form-registrar-prealerta').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function() {
                Swal.fire('Error', 'No se pudo registrar la prealerta.', 'error');
            }
        });
    });

    // ✅ EDITAR PREALERTA (CORREGIDO)
    $(document).on('submit', '.form-editar-prealerta', function(e) {
        ...
        // ✅ SOLO comparar campos editables, NO ocultos
        const camposActuales = {};
        const camposOriginales = $form.data('original') || {};
        
        $form.find('.campo-editable').each(function() {
            const name = $(this).attr('name');
            const value = $(this).val();
            camposActuales[name] = value;
        });

        // Si es la primera vez, guardar los valores originales
        if (Object.keys(camposOriginales).length === 0) {
            $form.data('original', camposActuales);
        } else {
            // Verificar cambios SOLO en campos editables
            let hayCambios = false;
            for (let campo in camposActuales) {
                if (camposActuales[campo] !== camposOriginales[campo]) {
                    hayCambios = true;
                    break;
                }
            }
            ...
            if (!hayCambios) {
                Swal.fire({
                    ...
                    text: 'No realizó ninguna modificación en la prealerta.',
                    timer: 1800,
                    showConfirmButton: false
                });
                return;
        ...
        $.ajax({
            ...
            error: function() {
                Swal.fire('Error', 'No se pudo actualizar la prealerta.', 'error');
            }
        });
    });
    ...
    $('#form-eliminar-prealerta').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function() {
                Swal.fire('Error', 'No se pudo eliminar la prealerta.', 'error');
            }
        });
    });
    ...
    function recargarTabla() {
        $.ajax({
            ...
            success: function(html) {
                ...
                $('body').append($html.find('.modal.fade[id^="edit-prealerta-"]'));
            },
            error: function() {
                console.error('Error recargando la tabla de prealertas');
            }
        });
    }
